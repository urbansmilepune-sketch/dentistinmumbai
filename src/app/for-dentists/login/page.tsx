...
async function handleGoogle() {
  ...
  // Step 2: verify the code. The server checks the code AND sets the session
  // cookie itself (see /api/auth/email-otp/verify), then returns { success }.
  // We hard-navigate to the landing page — a full top-level request so the
  // dashboard's SSR auth gate sees the just-set cookie (a soft RSC navigation
  // doesn't reliably deliver it to the first server render).
