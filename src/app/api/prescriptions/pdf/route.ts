...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: rx } = await supabase
      .from('prescriptions')
      .select('*, patients(name, age, phone, gender, allergies), dentists(name, clinic_name, phone, mci_number, address, areas(name))')
...
<div class="header">
  <div>
    ...
    <p>${dentist?.areas?.name ? dentist.areas.name + ', Mumbai' : ''} ${dentist?.phone ? '· ' + dentist.phone : ''}</p>
...
<div class="body">
  ...
  <div class="footer">
    <div style="font-size:12px;color:#64748b;">
      ...
      <p style="margin-top:4px;">Issued via dentistinmumbai.in</p>
    </div>
    <div class="signature">
      <div class="line"></div>
  ...
  <div class="watermark">Generated by dentistinmumbai.in · A Dentaura Prime LLP initiative</div>
</div>
</body>
</html>`
}
