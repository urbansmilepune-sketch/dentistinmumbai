...
function htmlResponse(title: string, message: string, ok: boolean, dashboardHref: string) {
  ...
  return new NextResponse(
    ...
    <body style="font-family: Arial, sans-serif; background: #f8faff; margin: 0; padding: 40px 20px;">
      <div style="max-width: 480px; margin: 0 auto; background: #fff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 40px 32px; text-align: center;">
        ...
        <a href="${dashboardHref}" style="display: inline-block; background: #0057A8; color: #fff; padding: 10px 22px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 14px;">Back to dashboard</a>
      </div>
    </body></html>`,
...
export async function GET(request: NextRequest) {
  // The dashboard link in the response is built from whatever host hit this
  // route — that's the city domain that owned the email, so it round-trips
  // the dentist back to the right brand.
  const host = request.headers.get('x-forwarded-host') || request.headers.get('host')
  const city = getCityByDomain(host)
  const dashboardHref = `https://${city.domain}/for-dentists/dashboard`

  const id = request.nextUrl.searchParams.get('id')
  if (!id) {
    return htmlResponse('Invalid link', 'This unsubscribe link is missing required info.', false, dashboardHref)
  }
  ...
  if (error || !dentist) {
    return htmlResponse('Could not unsubscribe', 'We could not find your account. Please email us if this keeps happening.', false, dashboardHref)
  }
